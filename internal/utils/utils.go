...
func GetCurrencySymbol(currencyCode string) string {
	if symbol, ok := CurrencySymbols[strings.ToUpper(currencyCode)]; ok {
		return symbol
	}
...
// GetFormattedCurrencyOptions generates a list for UI dropdowns like: "USD ($)", "EUR (€)"
func GetFormattedCurrencyOptions() []string {
	options := make([]string, len(CurrencyCodes))

	for i, code := range CurrencyCodes {
		symbol, exists := CurrencySymbols[code]
		if exists && symbol != "" {
			options[i] = fmt.Sprintf("%s (%s)", code, symbol)
		} else {
			options[i] = code // Fallback just in case a symbol is missing
		}
	}
	return options
}
